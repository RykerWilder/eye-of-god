...
const LOADING_LINES = (email) => [
  ...IDLE_LINES,
  { text: `> scanning ${email}...`,        color: 'text-[rgba(200,255,208,0.6)]' },
  { text: '> querying 120+ providers...',  color: 'text-[rgba(200,255,208,0.4)]' },
]
...
export default function OsintPage() {
  ...
  // Righe terminale derivate dallo stato
  const terminalLines = (() => {
    if (loading && result === null) return LOADING_LINES('')
    if (error)   return [...IDLE_LINES, { text: `> ERROR: ${error}`, color: 'text-red-400' }]
    if (result)  return [...IDLE_LINES, ...buildHoleheLines(result.email, result.data)]
    return IDLE_LINES
  })()
