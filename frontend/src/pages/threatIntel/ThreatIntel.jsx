...
function buildSherlockLines(username, data) {
  ...
  return [
    {
      text: "> SHERLOCK — Username Footprint",
      color: "text-[rgba(200,255,208,0.4)]",
    },
    { text: `> Target : ${username}`, color: "text-[rgba(200,255,208,0.7)]" },
    {
      text: `> Checked: ${data.length} platforms`,
      color: "text-[rgba(200,255,208,0.6)]",
    },
    {
      text: `> Found  : ${found.length} registered accounts`,
...
export default function OsintPage() {
  ...
  const terminalLines = (() => {
    ...
    if (result?.type === "sherlock") {
      return [
        ...IDLE_LINES,
        ...buildSherlockLines(result.username, result.data),
      ];
    }
