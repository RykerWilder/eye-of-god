...
import AbuseIPDB, {
  getAbuseIPDBLines,
  abuseIPDBLoadingLines,
} from "./tools/AbuseIPDB";
...
const IDLE_LINES = [
  { text: "> threat_intel module initialized.", color: "text-[rgba(200,255,208,0.3)]" },
...
function getLoadingLines(result) {
  if (!result) {
    const extra = TOOL_REGISTRY.flatMap((t) => t.loadingLines);
    return [...IDLE_LINES, ...extra];
  }
  const match = TOOL_REGISTRY.find((t) => t.getLines(result) !== null);
  return match
    ? [...IDLE_LINES, ...match.loadingLines]
    : IDLE_LINES;
}
